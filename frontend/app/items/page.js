...
export default function ItemsPage() {
  ...
  async function handleDelete() {
    ...
    try {
      await api(`/api/items/${itemToDelete.id}`, { method: 'DELETE' });
    ...
    <div className="app-layout">
      ...
      <main className="main-content">
        <div className="page-header">
          <div>
            <h2>Katalog Dekorasi</h2>
            <p>{items.length} item tersedia di katalog</p>
          </div>
          <Link href="/items/tambah" className="btn btn-primary">
        ...
        <div className="card">
          <div className="table-toolbar" style={{ borderBottom: 'none', padding: '0 0 var(--sp-5) 0' }}>
            <input
              type="text"
              placeholder="Cari item dekorasi..."
              value={search}
          ...
          {loading ? (
            ...
            <div className="table-empty" style={{ background: 'var(--bg-base)', borderRadius: 'var(--radius-sm)' }}>
              {search ? 'Item tidak ditemukan.' : 'Katalog masih kosong.'}
            </div>
          ) : (
